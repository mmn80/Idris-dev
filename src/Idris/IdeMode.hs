...
sexpToCommand :: SExp -> IdeModeCommand
sexpToCommand (SexpList ([x]))                                                              = sexpToCommand x
sexpToCommand (SexpList [SymbolAtom "interpret", StringAtom cmd])                           = Interpret cmd
sexpToCommand (SexpList [SymbolAtom "repl-completions", StringAtom prefix])                 = REPLCompletions prefix
sexpToCommand (SexpList [SymbolAtom "load-file", StringAtom filename, IntegerAtom line])    = LoadFile filename (Just (fromInteger line))
sexpToCommand (SexpList [SymbolAtom "load-file", StringAtom filename])                      = LoadFile filename Nothing
sexpToCommand (SexpList [SymbolAtom "type-of", StringAtom name])                            = TypeOf name
sexpToCommand (SexpList [SymbolAtom "case-split", IntegerAtom line, StringAtom name])       = CaseSplit (fromInteger line) name
sexpToCommand (SexpList [SymbolAtom "add-clause", IntegerAtom line, StringAtom name])       = AddClause (fromInteger line) name
sexpToCommand (SexpList [SymbolAtom "add-proof-clause", IntegerAtom line, StringAtom name]) = AddProofClause (fromInteger line) name
sexpToCommand (SexpList [SymbolAtom "add-missing", IntegerAtom line, StringAtom name])      = AddMissing (fromInteger line) name
sexpToCommand (SexpList [SymbolAtom "make-with", IntegerAtom line, StringAtom name])        = MakeWithBlock (fromInteger line) name
sexpToCommand (SexpList [SymbolAtom "make-case", IntegerAtom line, StringAtom name])        = MakeCaseBlock (fromInteger line) name
...
sexpToCommand (SexpList (SymbolAtom "proof-search" : IntegerAtom line : StringAtom name : rest))
  | [] <- rest = ProofSearch True (fromInteger line) name [] Nothing
  | [SexpList hintexp] <- rest
  ,  Just hints <- getHints hintexp = ProofSearch True (fromInteger line) name hints Nothing
  | [SexpList hintexp, IntegerAtom depth] <- rest
 , Just hints <- getHints hintexp = ProofSearch True (fromInteger line) name hints (Just (fromInteger depth))
 where getHints = mapM (\h -> case h of
                                StringAtom s -> Just s
                                _            -> Nothing)
sexpToCommand (SexpList [SymbolAtom "make-lemma", IntegerAtom line, StringAtom name])   = MakeLemma (fromInteger line) name
sexpToCommand (SexpList [SymbolAtom "refine", IntegerAtom line, StringAtom name, StringAtom hint]) = ProofSearch False (fromInteger line) name [hint] Nothing
sexpToCommand (SexpList [SymbolAtom "docs-for", StringAtom name])                       = DocsFor name Full
sexpToCommand (SexpList [SymbolAtom "docs-for", StringAtom name, SymbolAtom s])
  | Just w <- lookup s opts                                                             = DocsFor name w
    where opts = [("overview", Overview), ("full", Full)]
sexpToCommand (SexpList [SymbolAtom "apropos", StringAtom search])                      = Apropos search
sexpToCommand (SymbolAtom "get-options")                                                = GetOpts
sexpToCommand (SexpList [SymbolAtom "set-option", SymbolAtom s, BoolAtom b])
  | Just opt <- lookup s opts                                                           = SetOpt opt b
...
sexpToCommand (SexpList [SymbolAtom "metavariables", IntegerAtom cols])                 = Metavariables (fromIntegral cols)
sexpToCommand (SexpList [SymbolAtom "who-calls", StringAtom name])                      = WhoCalls name
sexpToCommand (SexpList [SymbolAtom "calls-who", StringAtom name])                      = CallsWho name
sexpToCommand (SexpList [SymbolAtom "browse-namespace", StringAtom ns])                 = BrowseNS ns
sexpToCommand (SexpList [SymbolAtom "normalise-term", StringAtom encoded])              = let (bnd, tm) = decodeTerm encoded in
                                                                                          TermNormalise bnd tm
sexpToCommand (SexpList [SymbolAtom "show-term-implicits", StringAtom encoded])         = let (bnd, tm) = decodeTerm encoded in
                                                                                          TermShowImplicits bnd tm
sexpToCommand (SexpList [SymbolAtom "hide-term-implicits", StringAtom encoded])         = let (bnd, tm) = decodeTerm encoded in
                                                                                          TermNoImplicits bnd tm
sexpToCommand (SexpList [SymbolAtom "elaborate-term", StringAtom encoded])              = let (bnd, tm) = decodeTerm encoded in
                                                                                          TermElab bnd tm
sexpToCommand (SexpList [SymbolAtom "print-definition", StringAtom name])               = PrintDef name
sexpToCommand (SexpList [SymbolAtom "error-string", StringAtom encoded])                = ErrString . decodeErr $ encoded
sexpToCommand (SexpList [SymbolAtom "error-pprint", StringAtom encoded])                = ErrPPrint . decodeErr $ encoded
sexpToCommand (SymbolAtom "version")                                                    = GetIdrisVersion
sexpToCommand sexp                                                                      = IdeExtension sexp
